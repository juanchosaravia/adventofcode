...
class day6Test() {
    ...
    @Test fun star12() {
        val matrix = Matrix<Int>(1000, 1000)
        for ((action, point1, point2) in commands) {
            matrix.loop(point1, point2) { x, y ->
                val value = matrix[x, y]
                when (action) {
                    LightAction.TURN_ON -> if (value == null) 1 else value + 1
                    LightAction.TURN_OFF -> if (value != null && value > 0) value - 1 else value
                    LightAction.TOGGLE -> if (value == null) 2 else value + 2
                }
            }
        }
        var countLightsOn = 0
    ...
    data class Point constructor(val x: Int, val y: Int) {
        companion object {
            inline operator fun <reified T> invoke(s: String): Point {
                val point = s.split(",")
                return Point(point.first().trim().toInt(), point.last().trim().toInt())
            }
        }
    }

    data class Command(val action: LightAction, val point1: Point, val point2: Point)

    /**
     * Found on stackoverflow, really useful!
     * I just added the function 'loop'
     */
    class Matrix<T>(val xSize: Int, val ySize: Int, val array: Array<Array<T>>) {
        ...
        inline fun loop(point1: Point, point2: Point, op: (Int, Int) -> T) {
