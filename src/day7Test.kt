 ...
 * Tool to test bitwise operations: http://www.convertforfree.com/bitwise-calculator/
 */
class day7Test() {
    ...
    val operationsMapValue = mutableMapOf<String, Int>()
    val stringOperationsMap = mutableMapOf<String, String>() // ID | STRING exp

    @Test fun testSampleData() {
        ...
        Assert.assertEquals(getValue("d"), 72)
        Assert.assertEquals(getValue("e"), 507)
        Assert.assertEquals(getValue("f"), 492)
        Assert.assertEquals(getValue("g"), 114)
        //Assert.assertEquals(getValue("h").getValue(), 65412)
        Assert.assertEquals(getValue("h"), -124)
        //Assert.assertEquals(getValue("i").getValue(), 65079)
        Assert.assertEquals(getValue("i"), -457)
        Assert.assertEquals(getValue("x"), 123)
        Assert.assertEquals(getValue("y"), 456)
        Assert.assertEquals(getValue("j"), -457)
        Assert.assertEquals(getValue("k"), -457)
    }

    @Test fun star13() {
        val result = getAValue()
    }

    @Test fun star14() {
        val aValue = getAValue()
        operationsMapValue.clear()
        operationsMapValue.put("b", aValue)
        val newValue = getAValue()
    }
    ...
    fun getValue(key: String): Int {
        if (!operationsMapValue.containsKey(key)) {
            try {
                return Expr.Number(key.toInt()).getValue()
            } catch (e: Exception) {
            }
            val expr = createExpr(stringOperationsMap[key]!!)
            operationsMapValue.put(key, expr.getValue())
        }
        return operationsMapValue[key]!!
    }
    ...
    sealed class Expr {
        ...
        class SingleAction(val e: Int, val op: SingleOperation) : Expr() {
            override fun getValue(): Int {
                val result = when (op) {
                    SingleOperation.NOT -> e.inv()
                }
                return result
            }

            override fun toString(): String {
                return "action: ${op.name} $e"
            }
        }

        class Action(val e1: Int, val e2: Int, val op: Operation) : Expr() {
            override fun getValue() = eval(e1, e2, op)
            fun eval(e1: Int, e2: Int, op: Operation): Int {
                val result = when (op) {
                    Operation.AND -> e1 and e2
                    Operation.OR -> e1 or e2
                    Operation.LSHIFT -> e1 shl e2
                    Operation.RSHIFT -> e1 shr e2
                }
                return result
            }

            override fun toString(): String {
                return "action: $e1 ${op.name} $e2"
            }
        }
    ...
    private fun createExpr(value: String): Expr {
        ...
        if (stringActions != null) {
            ...
            return Expr.Action(getValue(left), getValue(right), Operation.valueOf(operation))
        }
        ...
        if (stringActions != null) {
            ...
            return Expr.SingleAction(getValue(right), SingleOperation.valueOf(operation))
        }

        stringActions = getStringActions("(\\w+)", value)
        if (stringActions != null) {
            // it is a variable assigned to another variable
            return Expr.Number(getValue(stringActions[1]))
        }
